...
if [ -f "docs/contracts/_template.md" ]; then
  ...
  if [[ "$OSTYPE" == "darwin"* ]]; then
    ...
    sed -i '' "s|- 模块：|- 模块：${MODULE}|" "$FILENAME"
    sed -i '' "s|- 创建日期：|- 创建日期：${DATE}|" "$FILENAME"
    sed -i '' "s|状态：草稿 / 已确认 / 已实现 / 已废弃|状态：草稿|" "$FILENAME"
  else
    sed -i "s|# 行为契约：\[功能名称\]|# 行为契约：${FEATURE}|" "$FILENAME"
    sed -i "s|- 模块：|- 模块：${MODULE}|" "$FILENAME"
    sed -i "s|- 创建日期：|- 创建日期：${DATE}|" "$FILENAME"
  ...
  echo "# 行为契约：${FEATURE}" > "$FILENAME"
  echo "" >> "$FILENAME"
  echo "- 模块：${MODULE}" >> "$FILENAME"
  echo "- 创建日期：${DATE}" >> "$FILENAME"
  echo "- 状态：草稿" >> "$FILENAME"
  echo "- 关联 PR / Issue：" >> "$FILENAME"
fi
